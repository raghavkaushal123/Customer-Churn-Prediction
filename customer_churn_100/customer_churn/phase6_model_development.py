...
# Labels are assigned by thresholding predicted probability, the same way
# phase 7 does. pipeline.predict() is not equivalent for every model: for an
# SVC it uses the sign of the decision function, while predict_proba uses
# Platt scaling fitted by internal cross-validation, and the two disagree.
# Before this change the same SVM was reported at 0.7601 accuracy here and
# 0.7835 in phase 7. One explicit threshold makes every phase agree.
...
for model_name, pipeline in final_models.items():
    ...
    predicted_labels = (
        predicted_probabilities >= DECISION_THRESHOLD
    ).astype(int)
...
best_predictions = (
    best_probabilities >= DECISION_THRESHOLD
).astype(int)
...
print("\n11. PHASE 6 SUMMARY")
print(f"- Applied a stratified {1 - TEST_SIZE:.0%}/{TEST_SIZE:.0%} train-test split.")
